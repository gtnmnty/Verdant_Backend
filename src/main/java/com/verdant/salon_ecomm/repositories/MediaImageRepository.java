package com.verdant.salon_ecomm.repositories;

import com.verdant.salon_ecomm.entities.MediaImage;
import org.springframework.data.jpa.repository.JpaRepository;

import java.util.List;
import java.util.Optional;
import java.util.UUID;

public interface MediaImageRepository extends JpaRepository<MediaImage, UUID> {
    List<MediaImage> findByEntityTypeAndEntityId(String entityType, UUID entityId);
    Optional<MediaImage> findByEntityTypeAndEntityIdAndIsPrimaryTrue(String entityType, UUID entityId);
    void deleteByEntityTypeAndEntityId(String entityType, UUID entityId);
}
